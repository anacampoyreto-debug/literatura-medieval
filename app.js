...
function renderBlockTab(b, unlocked){
  ...
  if(state.tab==="cuaderno"){
    return `<div class="tab-content">
      <div class="tab-label">ESTÁS EN: CUADERNO</div>
  ...
  if(state.tab==="analisis"){
    return `<div class="tab-content">
      ...
      <div class="actions">
        ...
        <button class="btn btn-secondary" data-toggle-model="1">VER MODELO</button>
      </div>
      <div id="analysisResult"></div>
      <div class="model" id="analysisModel"><strong>MODELO:</strong> ${b.analysis_model}</div>
      <div class="feedback"><strong>MODELO PARA ESCRIBIR:</strong> ${b.writing_model}</div>
    </div>`;
  }
  if(state.tab==="quiz"){
    if(!unlocked){
    ...
    return `<div class="tab-content">
      <div class="tab-label">ESTÁS EN: QUIZ</div>
      <div class="quiz-list">
      ${b.quiz.map((q, idx) => {
  ...
  if(state.tab==="errores"){
    ...
    return `<div class="tab-content">
      <div class="tab-label">ESTÁS EN: ERRORES</div>
      ${mistakes.length ? mistakes.map(item => `<div class="mistake-card">
        <strong>Pregunta ${item.idx+1}</strong>
        <p>${item.q.q}</p>
        <p><strong>Correcta:</strong> ${String.fromCharCode(65+item.q.answer)}. ${item.q.options[item.q.answer]}</p>
        <p>${item.q.explanation}</p>
      </div>`).join("") : `<div class="card">Todavía no hay errores en este bloque.</div>`}
    </div>`;
  }
  return "";
}
...
function renderErrors(){
  ...
  return `<section class="panel">
    ...
    ${all.length ? all.map(item => `<div class="mistake-card" style="margin-bottom:12px">
      ...
      <p>Respuesta correcta: ${String.fromCharCode(65+item.q.answer)}. ${item.q.options[item.q.answer]}</p>
      <p>${item.q.explanation}</p>
    </div>`).join("") : `<div class="card">No hay errores guardados todavía.</div>`}
  </section>`;
}
    ...
    ${queue.length ? `<div class="review-list">
      ${queue.slice(0, 12).map((q, idx) => `
        <div class="quiz-card">
          ...
          <h4>${idx+1}. ${q.q}</h4>
          <div class="options">
            ${q.options.map((opt, optIdx) => `<button class="option-btn" data-review-idx="${idx}" data-review-opt="${optIdx}"><strong>${String.fromCharCode(65+optIdx)}.</strong> ${opt}</button>`).join("")}
          </div>
        </div>
      `).join("")}
...
function renderExam(){
  ...
  return `<section class="panel">
    ...
    <div class="actions">
      <button class="btn btn-gold" id="applyExamCount">APLICAR</button>
      <button class="btn btn-primary" id="submitExam">CORREGIR EXAMEN</button>
      <button class="btn btn-secondary" id="resetExam">REINICIAR</button>
    </div>
